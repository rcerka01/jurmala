package jurmala.api.directives.custom

import akka.http.scaladsl.model.{StatusCode, StatusCodes}
import akka.http.scaladsl.server.Directive1
import akka.http.scaladsl.server.Directives._
import akka.http.scaladsl.server.Directives.{parameters, provide, reject}
import jurmala.api.handlers.JurmalaRejection
import jurmala.domain.params.Paginate
...
trait PaginateDirectives {
  ...
  case class InvalidOffsetRejection(offset: Int) extends JurmalaRejection {
    val statusCode: StatusCode = StatusCodes.BadRequest
    val message = s"Invalid value for offset: '$offset'"
  }
  case class InvalidPageLimitRejection(maximumPageLimit: Int) extends JurmalaRejection {
    val statusCode: StatusCode = StatusCodes.BadRequest
